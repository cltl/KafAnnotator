 ...
 */
public class GetSentenceTagCorrelations {
    static final String NONE = "__NOVALUE";
    static boolean DEBUG = false;
    public static int[][] buildMatrix (HashMap<Integer, ArrayList<WordTag>> sentenceTagMap, ArrayList<String> tag1ArrayList, ArrayList<String> tag2ArrayList, int tag1, int tag2) {
        ...
        while (keys.hasNext()) {
            ...
            ArrayList<WordTag> tags = sentenceTagMap.get(key);
         //   System.out.println(key+" has tags = " + tags);
            ...
            for (int j = 0; j < tags.size(); j++) {
                WordTag tag = tags.get(j);
                String s1 = tag.getTag(tag1);
                String s2 = tag.getTag(tag2);
/*
                if (s2.equalsIgnoreCase("neutraal")) {
                    DEBUG = true;
...
*/
                ...
                if (idx1>=0) {
                    ...
                    if (!tag2Match) {
                        ...
                        idx2 = tag2ArrayList.indexOf(NONE);
                        if (idx2>=0) {
                            matrix[idx1][idx2]++;
                            if (DEBUG) {
                                System.out.println("idx1 ="+idx1+" and idx2 = " + idx2);
    ...
    static boolean hasTagCombination (ArrayList<WordTag> tags, String tag1Value, String tag2Value, int tag1, int tag2) {
        boolean hasIt = false;
        for (int i = 0; i < tags.size(); i++) {
            WordTag wordTag = tags.get(i);
            String v1 = wordTag.getTag(tag1);
            String v2 = wordTag.getTag(tag2);
            if ((v1.equals(tag1Value)) && (v2.equals(tag2Value))) {
                hasIt = true;
                break;
            }
        }
    ...
    static public void correlateFile(FileOutputStream fosFile, String filePath, int tag1, int tag2) throws IOException {
            String str ="\nCorrelation matrix tag level "+tag1+" and level "+tag2+"\n";
            fosFile.write(str.getBytes()); str = "";
            HashMap<Integer, ArrayList<WordTag>> sentenceTagMap = new HashMap<Integer, ArrayList<WordTag>>();
            ...
            while (keys.hasNext()) {
                ...
                if (sentenceId>0) {
                    if (sentenceTagMap.containsKey(sentenceId)) {
                        ArrayList<WordTag> tags = sentenceTagMap.get(sentenceId);
                        //// only unique and non-empty values are kept.
                        if ((tag1Value.length()>0) || (tag2Value.length()>0)) {
                            if (!hasTagCombination(tags, tag1Value, tag2Value, tag1, tag2)) {
                                tags.add(tag);
                    ...
                    //// we ignore sentence ID zero since the tags usually are assigned to other cases.
                }
            }
            ...
            //// We fill the matrix
            int [][] matrix = buildMatrix(sentenceTagMap, tag1ArrayList, tag2ArrayList, tag1, tag2);
